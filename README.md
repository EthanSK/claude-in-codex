...
- Codex 0.155 (tested here) tells the bridge which model is connecting in the WebSocket handshake. The bridge accepts GPT and Claude connections, then checks each message: Claude turns run locally, while GPT remains on WebSocket. A connection first opened for Claude only opens an upstream GPT socket if a later message needs one. Older clients without a model hint use HTTP. The bridge remains a local hop, and its latency versus a direct GPT connection has not been measured.
